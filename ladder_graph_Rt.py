...
@stop_watch
def get_actual_reliability(a, b, t):
    """
    :param a:ラダーの列
    :param b:ラダーの行
...
@stop_watch
def get_approximate_reliability(a, b, t, m) -> dict:
    un_rel, diff, failure = {}, {}, {}
    ladder = GraphLogic([a, b], t)
    truth_table = ladder.get_truth_table()
    shortest_path = ladder.get_shotest_path(a, b)
    in_root, out_root = ladder.approximate_status(truth_table, shortest_path)
    un_rel["min"], un_rel["max"] = ladder.approximate_reliability(in_root, out_root, m)
    un_rel["min"] = 1 - ladder.subs(un_rel["min"], "rel_min")
    un_rel["max"] = 1 - ladder.subs(un_rel["max"], "rel_max")
    return un_rel

@stop_watch
def get_fastest_approximate_reliability(a, b, t, m):
    ladder = GraphLogic([a, b], t)
    shortest_path = ladder.get_shotest_path(a, b)
...
for num in size:
    ...
    filename_time = f'3*{num}lab_{dt_now}'
    # actualの計算
    print(Fore.GREEN + f'【真値】故障率計算3*{num}')
    act[f"act_un_rel_3*{num}"] = 1 - get_actual_reliability(3, num, t)
    print('-----------------------------------------------------------')
    for j in range(2,3):
        # act エクセル出力
        ...
        for i in range(50):
            ...
            ws_value.cell(i + 2, 3, value=str(act[f"act_un_rel_3*{num}"].subs(t, i)))
        fig_rel, ax_rel = plt.subplots()
        plt.title(f'un_R(t)Max and un_R(t)min and actual [3×{num}]')
        plt.xlabel('t')
        plt.ylabel('un_reliability R(t)')
        # actualのプロット
        ax_rel.plot(t_value, [sp.lambdify(t, act[f"act_un_rel_3*{num}"])(i) for i in t_value], label=f'act_rel_3*{num}')
        # rel_appの計算とプロット
        print(Fore.YELLOW + f'【近似】故障率計算3*{num}、{j}個以上の故障ケース無視')
        app[f"app_un_rel_3*{num}_m{j}"]= get_approximate_reliability(3, num, t, j)
        print('-----------------------------------------------------------')
        # エクセルのヘッダー設定
        ...
        for i in range(50):
            # minとMaxをエクセルに書き込み
            ws_value.cell(i + 2, 2, value=str(app[f"app_un_rel_3*{num}_m{j}"]["min"].subs(t, i)))
            ws_value.cell(i + 2, 4, value=str(app[f"app_un_rel_3*{num}_m{j}"]["max"].subs(t, i)))
        ax_rel.plot(t_value, [sp.lambdify(t, app[f"app_un_rel_3*{num}_m{j}"]["min"])(i) for i in t_value], label=f'app_rel_3*{num}_m{j}_min')
        ax_rel.plot(t_value, [sp.lambdify(t, app[f"app_un_rel_3*{num}_m{j}"]["max"])(i) for i in t_value], label=f'app_rel_3*{num}_m{j}_max')
        # fail_appの計算とプロット
        ax_rel.legend()
